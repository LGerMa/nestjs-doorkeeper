...
### Added
- Exported `Public`, `CurrentUser`, `JwtAuthGuard`, `RefreshGuard`, `UserEntity`, `SessionEntity` from the root package entry point
...
### Fixed
- Added explicit `@Inject()` decorators to all constructor parameters across `AuthController`, `JwtAuthGuard`, `AuthService`, `SessionService`, and `TokenService` — ensures reliable dependency injection when the library is used as a compiled package
...
### Fixed
- Removed `TypeOrmModule.forFeature()` from `AuthModule` — it caused NestJS to hang during bootstrap due to a conflict with `applyTablePrefix` metadata mutation. The `TypeOrmAdapter` uses `dataSource.getRepository()` directly so it was never needed.
